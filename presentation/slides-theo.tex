...
\begin{frame}{Prior Work: Vision-Based Parkour}
	\begin{columns}[T]
		...
		\includemedia[
			...
			activate=onclick,
			addresource=pictures/extreme-parkour.mp4,
			flashvars={
				source=pictures/extreme-parkour.mp4
				&loop=true
				&autoPlay=true
			}
		]{}{VPlayer.swf}
...
\begin{frame}{Simulation Infrastructure}
	\begin{columns}[c]
		...
		\end{column}
		\begin{column}{0.48\linewidth}
			\includegraphics[width=\linewidth]{genesis.jpg}
			\vspace{4pt}
			{\small \textbf{Genesis} \textcolor{gray}{\scriptsize[Genesis Authors, 2024]}\\GPU-accelerated physics simulator}
		\end{column}
	\end{columns}
...
\begin{frame}{Two-Stage Distillation Pipeline}
	...
	\end{center}
	\vspace{2pt}
	{\small Adapted from Extreme Parkour \textcolor{gray}{\scriptsize[Cheng et al.\ 2023]}}
...
% --------------------------------------------------------------------------
% SCRIPT: "Let me show you what this looks like in practice. This is the teacher
% policy running with privileged scandot access --- it can see the terrain geometry
% directly, which is why it clears the gap so cleanly."
\begin{frame}{Teacher Policy Demo}

	\begin{center}
		\includemedia[
			width=0.85\linewidth,height=0.80\textheight,
			activate=onclick,
			addresource=pictures/teacher_1env_back_d3_tex.mp4,
			flashvars={
				source=pictures/teacher_1env_back_d3_tex.mp4
				&loop=true
				&autoPlay=true
			}
		]{}{VPlayer.swf}
...
% --------------------------------------------------------------------------
% SCRIPT: "Here's how the three students trained. The GT-depth student converges
% fastest and reaches the highest reward --- that's our upper bound. DA2-base with
% texture tracks closely behind. DA2-base without texture lags significantly,
% confirming that texture is the key factor. The distillation loss tells the same
% story --- GT-depth achieves the lowest action imitation error, with textured DA2
% closing most of the gap."
\begin{frame}{Student Training Dynamics}

	\begin{center}
		\includegraphics[width=0.88\linewidth]{student_success_comparison.png}
	\end{center}
...
% --------------------------------------------------------------------------
% SCRIPT: "And here is the DA2 student --- same course, but now using only an
% RGB camera and Depth Anything V2 estimates. The policy transfers remarkably
% well on textured terrain. This is the result we set out to achieve."
\begin{frame}{DA2 Student Demo}

	\begin{center}
		\includemedia[
			width=0.85\linewidth,height=0.80\textheight,
			activate=onclick,
			addresource=pictures/da2student_1env_front_d0_tex.mp4,
			flashvars={
				source=pictures/da2student_1env_front_d0_tex.mp4
				&loop=true
				&autoPlay=true
			}
		]{}{VPlayer.swf}
...
% SCRIPT: "Here are the numbers behind that scatter plot. The baseline with
% checkerboard texture is at the top. Realistic textures like stone tiles hold
% up well, but concrete and gravel degrade gaps significantly. The critical
% failure mode is featureless terrain --- gaps drop to zero. Lighting and color
% shifts are surprisingly mild. The combined perturbations show compounding:
% mild combination is manageable, extreme combination is catastrophic for gaps."
\begin{frame}{Visual Sensitivity}
...
\begin{tabular}{@{}l rr rr rr@{}}
  ...
  \midrule
  Baseline            & 96   & 94   & 100  & 92   & 97   & 95   \\
  \midrule
  Stone tiles         & 93   & 79   & 100  & 97   & 95   & 91   \\
  Concrete            & 79   & 12   & 100  & 26   & 96   & 73   \\
  No texture          & 17   &  0   & 100  & 22   & 91   & 18   \\
  \midrule
  Dim (0.3$\times$)   & 83   & 58   & 100  & 94   & 100  & 96   \\
  Color $\pm$30\%     & 79   & 32   & 100  & 95   & 97   & 88   \\
  \midrule
  Combined (mild)     & 77   &  8   & 100  & 15   & 97   & 79   \\
  Combined (extreme)  & 34   &  0   & 100  & 67   & 90   & 19   \\
  \bottomrule
\end{tabular}
\end{center}
...
\column{0.5\textwidth}
  ...
  \begin{itemize}
    \setlength\itemsep{0.45em}
		\item Monocular depth is a plausible bridge from RGB to locomotion
    \item Moderate robustness to photometric variation without domain randomization
    \item Bottleneck is depth quality, not student architecture
...
\column{0.5\textwidth}
  ...
  \begin{itemize}
    \setlength\itemsep{0.45em}
    \item Simulation only, no hardware validation yet. Needs physics domain randomization.
