...
\textbf{Note:} The above results used \texttt{texture\_uv\_scale=10.0}, which made
procedural textures tile so densely they appeared as near-solid colours from the
robot's camera.  After correcting the UV scale to 4.0 (visible patterns), the
retrained model (10{,}000 iterations, same hyperparameters) achieves:
...
\midrule
BASE3 @10000 (uv=4.0) & 64.8 & 64.1 & 68.0 & 44.9 \\
\bottomrule
\end{tabular}
\end{center}

The drop is caused by DA2 interpreting bold texture patterns as spurious depth
variation (DSQ decreases by $\approx$1.5\% Pearson correlation relative to
invisible textures).  This is the honest result with correctly-scaled textures.
...
\best{BASE3 @10000 (uv=4)} & \best{64.8} & \best{64.1} & \best{68.0} & \best{44.9} \\
Gap to GT            & $-$29.7 & $-$30.4 & $-$21.1 & $-$42.6 \\
\bottomrule
\end{tabular}
\end{center}

The gap to GT-depth is substantially larger with correctly-scaled textures.
Row~3 (0.50\,m gaps) improved from 0.8\% (DA2-base alone) to 44.9\% (DA2-base +
texture at 10{,}000 iterations), but remains 42.6\,pp below the GT-depth
student's 87.5\%.
