...
Through domain invariance experiments, we show that DA2 provides strong robustness to
  color and brightness changes and generalizes across simulated environments.
\end{abstract}
...
gaps---must perceive geometry. In recent years, training locomotion policies in simulation using reinforcement learning (RL) then transferring them to real robots has proven highly effective in mastering agile skills~\citep{lee2020learning, bellegarda2020robust, fu2021minimizing,margolis2021learning, rudin2022learning, bellegarda2022robust,li2023robust, he2024agile,inoue2024body, ma2024dreureka}.

Notably, recent work~\citep{caluwaerts2023barkour, zhuang2023robot, cheng2023parkour, hoeller2024anymal, chane2025soloparkour} has shown that RL combined with teacher--student distillation can produce quadruped controllers capable of dynamic skills akin to the athletic maneuvers seen in parkour, including walking, climbing, jumping, and crawling, which necessitates tight coordination between vision and control. 

 In simulation, privileged information such as ground-truth heightmaps makes parkour straightforward, but real robots lack such oracles. 
To address this, these systems follow a two-phase paradigm: a \emph{teacher} trains with privileged terrain information, then a \emph{student} reproduces the teacher's behavior from realistic sensors.
...
We propose an application of strategy~(3): \emph{using a pre-trained monocular depth
estimator as a fixed intermediate representation}. 
If a depth estimator produces structurally consistent depth from both simulated and real RGB, its output can be used as a domain-invariant canonical representation. 

Specifically, foundation models such as Depth~Anything~V2 (DA2) \cite{yang2024depth} have been trained on millions of diverse real images, learning
a mapping from arbitrary RGB to a canonical metric depth space. Because this
...
\begin{itemize}[nosep,leftmargin=*]
  \item A complete two-stage teacher--student pipeline from privileged scandot
    heightmaps to monocular depth for quadruped parkour.
  \item A functional demonstration of monocular depth estimation as a viable vision pipeline for quadruped parkour, achieving basic performance on simulated stairs, hurdles, and gaps.
  \item Systematic identification of depth signal quality---governed by
    model scale and terrain texture---as the key factor impacting parkour performance.
